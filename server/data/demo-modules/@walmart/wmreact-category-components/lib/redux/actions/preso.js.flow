...
import { buildPresoUri, isEmptyModules } from "../helpers/preso";
...
export const bootstrapModules = (params) => (dispatch) =>
  fetchJSON(buildPresoUri(params), getFetchOptions(params.req))
    .then((response) => {
      ...
      if (response.redirectUrl || isEmptyModules(response.modules)) {
        ...
        const redirectUrl = response.redirectUrl || `/browse/${params.categoryId}`;
