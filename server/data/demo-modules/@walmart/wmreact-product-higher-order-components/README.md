product-higher-order-components
==================

A set of react components, including DetailedHeroImage, AboutItem, ProductsCarousel, ProductsGrid, Comparison, Tile
...
```
npm install @walmart/wmreact-product-higher-order-components
```
...
- Run `builder run test` and ensure it passes. This will lint your js code and run unit tests.
...
specific css selectors with a `prod-` name space, for e.g. the style class for Carousel section could be `prod-ProductsCarousel`.
