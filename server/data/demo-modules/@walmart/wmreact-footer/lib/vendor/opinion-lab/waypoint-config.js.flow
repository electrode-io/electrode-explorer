 ...
 */
 /*eslint-disable */
import waypointEngine from "./waypoint-engine";

export default {
  // Builds the custom variables used by OpinionLab in a format they require
  generateOpinionLabVars: function () {    
    var vars = {},
    /*jshint camelcase:false */
      s_vi = waypointEngine.readCookie("s_vi"),
    /*jshint camelcase:true */
      bstc = waypointEngine.readCookie("bstc"),
      cartId = waypointEngine.readCookie("WMSessionID"), // more parsing required
      visitorId = waypointEngine.readCookie("com.wm.anoncart"),
      jSessionId = waypointEngine.readCookie("com.wm.visitor"),
      serverName = document.getElementsByClassName("env-info")
        && document.getElementsByClassName("env-info")[0], // more parsing required
      ACID = waypointEngine.readCookie("ACID"),
      CRT = waypointEngine.readCookie("CRT"),
      PCID = waypointEngine.readCookie("PCID"),
      CID = waypointEngine.readCookie("CID"),
      referenceId = document.getElementsByClassName("reference-id")
        && document.getElementsByClassName("reference-id")[0]; // more parsing required
    ...
    if (typeof s_omni !== "undefined") {
      ...
      vars.checkoutFulfillmentMethod =
        typeof s_omni.eVar31 !== "undefined" ? s_omni.eVar31 : "null";
  ...
  waypointInit: function (path) {
    /*jshint camelcase:false */
    waypointEngine.oo_waypoint = new waypointEngine.Waypoint({
      ...
      categories: {
        Website_feedback: {
          oCode: {
            legacyVariables: {
              vars: waypointEngine.readCookie("bstc"),
              override: true
            },
