...
const orchestrate = function(root) {
  const OOo = {
  ...
  (function () {
    ...
    function hideWaypoint(e, lf) {
      ...
      var wpContainer = OOo.$('oo_waypoint_container'), parent = wpContainer.parentNode;
    ...
    function getWaypoint() {
      ...
      var resp = '<div id=\"oo_waypoint_prompt\" role=\"dialogue\" aria-describedby=\"'
      + 'oo_waypoint_message\"><div id=\"oo_company_logo\"></div><div id=\"oo_waypoint_content\">'
      + '<p id=\"oo_waypoint_message\"><span class=\"top\">Give us your feedback</span><br />'
      + 'so we can serve you better.</p><p id=\"waypoint_icons\"></p><p id=\"ol_brand_logo\">'
      + '<a href=\"//www.opinionlab.com/company/\" target=\"_blank\" aria-label=\"'
      + 'Powered by OpinionLab. This will open a new window\"></a></p></div>'
      + '<a id=\"oo_close_prompt\" href=\"#\" aria-label=\"Close dialog\">'
      + '<div class=\"screen_reader\">Close dialogue</div>'
    ...
    function showWaypoint(markup) {
      ...
      for (var key in categories) {
        if (categories.hasOwnProperty(key)) {
          ...
          newLink.href = '#';
          newLink.innerHTML = String(key).replace(/_/g, ' ')
          ...
          if (typeof opt.categories[key].oCode === 'string') {
            OOo.addEventListener(newLink, 'click', (function (key) {
              return function (e) {
                var evt = e || window.event;
                window.open(opt.categories[key].oCode,
                  '_blank', 'scrollbars=yes,location=yes,menubar=yes,resizable=yes');
  ...
  OOo.Ocode = function (options) {
    ...
    if (opt.tunnel
        && !OOo.checkTunnel(location.pathname, opt.tunnel.path, opt.tunnel.cookieName)) {
      return;
    }
  ...
  (function () {
    ...
    function launchOOPopup() {
      var
        ...
        form, win,
        popupOptions = 'resizable=yes,location=no,status=no,scrollbars=1,width='
          + popupDimensions[0] + ',height=' + popupDimensions[1] + ',top='
          + popupCoordinates[0] + ',left=' + popupCoordinates[1],
      ...
      var winUrl = 'https://secure.opinionlab.com/ccc01/comment_card_d.asp?'
  ...
  (function () {
    function setupEvents() {
      ...
      for (j = eventTypes.length - 1; j >= 0; j -= 1) {
        ...
        if (events[eventType] instanceof Array) {
          ...
          while (i && !addEvent[j]) {
            i -= 1;
            if (window.location.href.search(eventObj[i].url) !== -1
              && Math.random() >= 1 - eventObj[i].p / 100) {
              addEvent[j] = true;
            }
          }
    ...
    function disableElements(e) {
      var evt = e || window.event, tar = e.target
        || e.srcElement, events = this.options.events
        , parent = tar.parentNode, MAXDEPTH = 5, depth = 0;
      while (parent
          && (tar.nodeName !== 'A' || tar.nodeName !== 'INPUT')
          && depth !== MAXDEPTH) {
      ...
      if (events.disableFormElements
          && (tar.tagName === "INPUT" || tar.tagName === "BUTTON")
          && (tar.type === 'submit'
              || tar.type === 'image'
              || tar.type === 'reset'
              || tar.type === 'button')) {
      ...
      if (events.disableLinks
          && (tar.nodeName === 'A' || tar.nodeName === 'AREA')
          && tar.href.substr(0, 4) === 'http'
          && tar.href.search(events.disableLinks) !== -1) {
        this.interruptShow = true;
      }
    }
        ...
        switch (opt.position) {
          ...
          case 'fixedPreserveContent':
            adjustLogo = function (e) {
              ...
              if (winWidth <= getRightOfContent(contentEl)
                  + wrapper.offsetWidth
                  + parseInt(logoOffset, 10)) {
  ...
  OOo.extend(OOo.Ocode.prototype, {
    bar: function () {
      var d = document,
        ...
        adjustBar, initialHeight, amount,
      ...
      function checkTabThrough(e) {
        ...
        if (!offset) return;
        if (offset[1] + activeElement.clientHeight > (window.innerHeight
              || document.body.clientHeight) +
              (window.pageYOffset || document.body.scrollTop) - wrapper.clientHeight) {
  ...
  OOo.extend(OOo.Ocode.prototype, {
    tab: function () {
      ...
      function ios4Logo(e) {
        ...
        wrapper.style.right = document.documentElement.clientWidth
          - window.innerWidth - window.pageXOffset - 15 + 'px';
      }
  ...
  OOo.extend(OOo.Ocode.prototype, {
    setupOnPageCC: function () {
      var
        ...
        frame = d.createElement(OOo.DYNAMIC_FRAME_NAME_IS_BUGGY
      ...
      close.innerHTML = '<span class="screen_reader">'
        + 'Close dialog</span><span aria-hidden="true">&#10006;</span>';
      ...
      if (OOo.Browser.IE) {
        ...
        if (!window.XMLHttpRequest || d.compatMode === "BackCompat") {
          ...
          overlay.style.height =
            Math.max(d.documentElement.clientHeight, d.body.offsetHeight) + 'px';
          wrapper.style.position = 'absolute';
          OOo.addEventListener(window, 'scroll', function () {
            overlay.style.top = (d.body.scrollTop
                + document.body.clientHeight
  ...
  OOo.extend(OOo.Ocode, {
    postMessageHandler: function (callback, commentCardUrl, loc) {
      return function (e) {
        ...
        if ((e && !(e.origin === messageURL || e.origin.indexOf(commentCardUrl) !== 0))
            || (!e && loc.hash.search('OL=') === -1)) {
          return false;
        }
  ...
  OOo.Invitation = function (options) {
    ...
    if (!window.OOoDynamicRewrite
        || window.OOoDynamicRewrite.weight < cookieOODynamicRewrite.weight) {
    ...
    if (window.OOoDynamicRewrite
      ...
      if (window.OOoDynamicRewrite.searchPattern) {
        OOo.createCookie('oo_OODynamicRewrite_searchPattern',
          window.OOoDynamicRewrite.searchPattern);
      }
      if (window.OOoDynamicRewrite.replacePattern) {
        OOo.createCookie('oo_OODynamicRewrite_replacePattern',
    ...
    if (!OOo.readCookie(opt.percentageCookie)) {
      OOo.createCookie(opt.percentageCookie,
  ...
  OOo.Invitation.prototype = {
    ...
    launchPopup: function (e) {
      ...
      var
        ...
        url = opt.popupType === 'popup'
          ? 'https://secure.opinionlab.com/ccc01/comment_card.asp?'
          : opt.pathToAssets + opt.monitorWindow + '?time1=' + (new Date()).getTime() + '&',
        urlParams,
        customVars = [],
      ...
      if (opt.clickTalePID && window.ClickTaleGetUID && window.ClickTaleGetSID) {
        prependCustomVar += '|'
          + [opt.clickTalePID, window.ClickTaleGetUID(), window.ClickTaleGetSID()].join('/');
      }

      popupDimensions = opt.newWindowSize || popupDimensions;
      popupOptions = 'scrollbars=1,resizable=1,location=no,status=no,width='
        + popupDimensions[0] + ',height=' + popupDimensions[1];
  ...
  OOo.Waypoint = function (options) {
    ...
    var opt = this.options, categories = opt.categories;

    for (var key in categories) {
      if (categories.hasOwnProperty(key)) {
        if (typeof opt.categories[key].oCode === 'object') {
          var extension = {};
  ...
  OOo.Waypoint.prototype = {
    ...
    showWaypoint: function (markup) {
      OOo.showWaypoint.call(this, markup);
    },
    killWaypoint: function (e) {
      var evt = e || window.event;
