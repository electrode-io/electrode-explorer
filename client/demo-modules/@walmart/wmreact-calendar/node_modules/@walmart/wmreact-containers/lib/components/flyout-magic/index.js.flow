...
class Flyout extends Component {
  ...
  updateTarget() : void {
    ...
    const target = ReactDOM.findDOMNode(this.refs.trigger);
  ...
  renderTrigger() : React.Element {
    const { trigger: Trigger, hover } = this.props;
    ...
    // Legacy way of doing things. We have no idea what the trigger is,
    // just assume it's something that supports `onClick` and hope for the
    // best. YOLO.
    const handlers = {
      onClick: () => this.toggle()
    };
    if (hover) {
      handlers.onMouseEnter = () => this.show();
    }
    return React.cloneElement(Trigger, handlers);
  }
